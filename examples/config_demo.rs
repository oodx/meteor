...
fn main() {
    ...
    println!("- Max meteors per shower: {}", config::MAX_METEORS_PER_SHOWER);
    ...
    println!("- Max token value length: {}", config::MAX_TOKEN_VALUE_LENGTH);
    println!();

    println!("Security Features:");
    println!("- Runtime tampering prevention: {}", config::prevent_runtime_tampering());
    println!("- Namespace character validation: {}", config::validate_namespace_characters());
    println!("- Reserved word enforcement: {}", config::enforce_reserved_words());
    println!("- Command audit trail: {}", config::enable_command_audit_trail());
    println!();
    ...
    println!("- Cannot exceed {} meteors per shower", config::MAX_METEORS_PER_SHOWER);
    println!("- Cannot exceed {} command history entries", config::MAX_COMMAND_HISTORY);
    println!("- Cannot create more than {} contexts", config::MAX_CONTEXTS);
    println!("- Token keys limited to {} characters", config::MAX_TOKEN_KEY_LENGTH);
    println!("- Token values limited to {} characters", config::MAX_TOKEN_VALUE_LENGTH);
    println!();
