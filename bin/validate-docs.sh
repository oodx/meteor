#!/bin/bash
#===============================================================================
# 🔍 METEOR DOCUMENTATION VALIDATOR
#===============================================================================
#
...
echo "🔍 Validating Meteor documentation integrity..."
echo
...
check_file_exists "docs/ref/RSB_TOKEN_REFERENCE.md" "RSB migration guide"
check_file_exists "docs/ref/TOKEN_NAMESPACE_CONCEPT.md" "Namespace concept guide"
...
if [[ -d ".analysis" ]]; then
    ...
    check_file_exists ".analysis/mvp_triage.txt" "MVP triage results"
else
...
if [[ ! -f "bin/test.sh" ]]; then
    warn "bin/test.sh missing (RSB compliance requirement)"
fi

if [[ ! -d "tests" ]]; then
    warn "tests/ directory missing (RSB compliance requirement)"
fi
